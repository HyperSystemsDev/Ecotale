...
public class EcoAdminGui extends InteractiveCustomUIPage<EcoAdminGui.AdminGuiData> {
    ...
    private static final List<String> AVAILABLE_LANGUAGES = List.of(
        "en-US", "es-ES", "pt-BR", "fr-FR", "de-DE", "ru-RU", "tr-TR", "ja-JP", "zh-CN"
    );
    ...
    // Name cache - loaded once on open to avoid repeated DB queries
    ...
    public EcoAdminGui(@NonNullDecl PlayerRef playerRef) {
        ...
        // Load all known player names from centralized service
        var nameService = com.ecotale.util.PlayerNameService.getInstance();
        this.nameCache = (nameService != null) ? new java.util.HashMap<>(nameService.getAllCached()) : new java.util.HashMap<>();
    }
    ...
    public void build(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl UICommandBuilder cmd, 
        ...
        events.addEventBinding(CustomUIEventBindingType.Activating, "#HudTranslationToggle", 
            EventData.of("Action", "ToggleHudTranslation"), false);
        events.addEventBinding(CustomUIEventBindingType.Activating, "#SymbolPositionToggle", 
            EventData.of("Action", "ToggleSymbolPosition"), false);
    ...
    public void handleDataEvent(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl Store<EntityStore> store, 
        ...
        if (data.action != null) {
            ...
            switch (data.action) {
                ...
                case "ToggleHudTranslation" -> {
                    var config = Main.CONFIG.get();
                    boolean newValue = !config.isUseHudTranslation();
                    config.setUseHudTranslation(newValue);
                    BalanceHudSystem.refreshAllHuds(); // Apply immediately
                    playerRef.sendMessage(Message.raw("HUD Translation: " + (newValue ? "Enabled" : "Disabled")).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "ToggleSymbolPosition" -> {
                    var config = Main.CONFIG.get();
                    boolean newValue = !config.isSymbolOnRight();
                    config.setSymbolOnRight(newValue);
                    BalanceHudSystem.refreshAllHuds(); // Apply immediately to show new format
                    playerRef.sendMessage(Message.raw("Symbol Position: " + (newValue ? "Right" : "Left")).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "ResetDefaults" -> {
                    ...
                    config.setHudPrefix("Balance");
                    config.setStartingBalance(100);
                    config.setMaxBalance(1_000_000_000);
                    config.setDecimalPlaces(2);
                    config.setEnableHudDisplay(true);
                    config.setUseHudTranslation(false); // Default: use raw config value
                    config.setSymbolOnRight(false); // Default: symbol on left
    ...
    private void buildConfigTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        // Display fields - use I18nModule for translation
        String yesText = getTranslation(lang, "ecotale.gui.config.yes", "Yes");
        String noText = getTranslation(lang, "ecotale.gui.config.no", "No");
        ...
        // HUD Translation toggle (show YES/NO based on useHudTranslation)
        cmd.set("#HudTranslationToggle.Text", config.isUseHudTranslation() ? yesText : noText);
        
        // Symbol Position toggle (show LEFT/RIGHT)
        String leftText = getTranslation(lang, "ecotale.gui.config.left", "LEFT");
        String rightText = getTranslation(lang, "ecotale.gui.config.right", "RIGHT");
        cmd.set("#SymbolPositionToggle.Text", config.isSymbolOnRight() ? rightText : leftText);
    }
    
    /** Helper to get translated text with fallback */
    private String getTranslation(String lang, String key, String fallback) {
        String value = I18nModule.get().getMessage(lang, key);
    ...
    /** Translate all static UI elements based on server config language */
    private void translateUI(@NonNullDecl UICommandBuilder cmd) {
        String lang = Main.CONFIG.get().getLanguage();
        
        // Header
        cmd.set("#Title.Text", getTranslation(lang, "ecotale.gui.admin.title", "Economy Admin Panel"));
        cmd.set("#ForceSaveButton.Text", getTranslation(lang, "ecotale.gui.admin.force_save", "FORCE SAVE"));
        
        // Tab buttons
        cmd.set("#TabDashboard.Text", getTranslation(lang, "ecotale.gui.admin.tab.dashboard", "DASHBOARD"));
        cmd.set("#TabPlayers.Text", getTranslation(lang, "ecotale.gui.admin.tab.players", "PLAYERS"));
        cmd.set("#TabTop.Text", getTranslation(lang, "ecotale.gui.admin.tab.top", "TOP"));
        cmd.set("#TabLog.Text", getTranslation(lang, "ecotale.gui.admin.tab.log", "LOG"));
        cmd.set("#TabConfig.Text", getTranslation(lang, "ecotale.gui.admin.tab.config", "CONFIG"));
        
        // Dashboard labels
        cmd.set("#LblTotalCirculating.Text", getTranslation(lang, "ecotale.gui.dashboard.total_circulating", "Total Circulating"));
        cmd.set("#LblPlayersWithBalance.Text", getTranslation(lang, "ecotale.gui.dashboard.total_players", "Players with Balance"));
        cmd.set("#LblAverageBalance.Text", getTranslation(lang, "ecotale.gui.dashboard.avg_balance", "Average Balance"));
        cmd.set("#LblCurrentConfig.Text", getTranslation(lang, "ecotale.gui.dashboard.current_config", "Current Configuration"));
        cmd.set("#LblRecentActivity.Text", getTranslation(lang, "ecotale.gui.dashboard.recent_activity", "Recent Activity"));
        
        // Players tab
        cmd.set("#LblSearch.Text", getTranslation(lang, "ecotale.gui.players.search_label", "Search"));
        cmd.set("#LblAmount.Text", getTranslation(lang, "ecotale.gui.players.amount", "Amount:"));
        cmd.set("#SelectedLabel.Text", getTranslation(lang, "ecotale.gui.players.select_player", "Select a player"));
        cmd.set("#GiveButton.Text", getTranslation(lang, "ecotale.gui.players.give", "GIVE"));
        cmd.set("#TakeButton.Text", getTranslation(lang, "ecotale.gui.players.take", "TAKE"));
        cmd.set("#SetButton.Text", getTranslation(lang, "ecotale.gui.players.set", "SET"));
        cmd.set("#ResetButton.Text", getTranslation(lang, "ecotale.gui.players.reset", "RESET"));
        
        // Config tab section headers (only these have IDs)
        cmd.set("#LblCurrency.Text", getTranslation(lang, "ecotale.gui.config.currency", "Currency"));
        cmd.set("#LblLimits.Text", getTranslation(lang, "ecotale.gui.config.limits", "Balance Limits"));
        cmd.set("#LblDisplay.Text", getTranslation(lang, "ecotale.gui.config.display", "Display"));
        cmd.set("#LblLanguage.Text", getTranslation(lang, "ecotale.gui.config.language", "Language"));
        
        // Config tab buttons
        cmd.set("#SaveConfigButton.Text", getTranslation(lang, "ecotale.gui.config.save", "SAVE CONFIG"));
        cmd.set("#ResetDefaultsButton.Text", getTranslation(lang, "ecotale.gui.config.reset_defaults", "RESET DEFAULTS"));
    }
    ...
    private String getPlayerName(UUID uuid) {
        // 1. Check online players first (most up-to-date)
        PlayerRef onlinePlayer = Universe.get().getPlayer(uuid);
        if (onlinePlayer != null) {
            return onlinePlayer.getUsername();
        }
        // 2. Check name cache (loaded from DB on GUI open)
