...
public class JsonStorageProvider implements StorageProvider {
    ...
    private Path getSettingsFile(UUID uuid) {
        return PLAYERS_PATH.resolve(uuid.toString() + "_settings.json");
    }
    @Override
    public CompletableFuture<Boolean> getHudVisible(@Nonnull UUID playerUuid) {
        return CompletableFuture.supplyAsync(() -> getHudVisibleSync(playerUuid));
    }
    
    /**
     * Synchronous version for use during player join.
     */
    public boolean getHudVisibleSync(@Nonnull UUID playerUuid) {
        Path settingsFile = getSettingsFile(playerUuid);
        ...
        try {
            ...
            // Simple JSON parsing: {"hudVisible":false}
            return !content.contains("\"hudVisible\":false");
        } catch (IOException e) {
            logger.at(Level.WARNING).log("Failed to read settings for %s: %s", playerUuid, e.getMessage());
            return true;
        }
    }
    ...
    public CompletableFuture<Void> setHudVisible(@Nonnull UUID playerUuid, boolean visible) {
        return CompletableFuture.runAsync(() -> {
            Path settingsFile = getSettingsFile(playerUuid);
            try {
                String json = "{\"hudVisible\":" + visible + "}";
                Files.writeString(settingsFile, json);
            } catch (IOException e) {
                logger.at(Level.WARNING).log("Failed to save settings for %s: %s", playerUuid, e.getMessage());
            }
        });
    }
    /**
